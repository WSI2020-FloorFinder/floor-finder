...
async function description(description_URL) {
  ...
  for (let i = 0; i < description_URL.length; i++) {
    ...
    if (description_URL[i] === 'https://afrotc.iit.edu/') {
    ...
    else if (description_URL[i] === "https://www.iit.edu/stochastic-dynamics"){
      document.querySelectorAll("p").forEach(e => {
      ...
      document.querySelectorAll("p").forEach(e => {
        if (e.textContent.includes("The Office of Human Resources is dedicated to")){
          descriptions.push(e.textContent.replace(/\n/g, ' '))
        }
      })
    }
    else if (description_URL[i] === "https://www.iit.edu/student-accounting"){
      document.querySelectorAll("p").forEach(e => {
        if (e.textContent.includes("The Office of Student Accounting is here")){
          descriptions.push(e.textContent.replace(/\n/g, ' '))
        }
    })
    }
