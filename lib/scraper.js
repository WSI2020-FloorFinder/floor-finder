...
async function description(description_URL){
    ...
    for(let i = 0; i < description_URL.length; i++){
        ...
        if (description_URL[i] == 'https://afrotc.iit.edu/'){
            ...
            descriptions.push(document.querySelector('.field-item').textContent.replace(/\n/g, ' '));
        }
        ...
        else if (description_URL[i] == 'https://www.iit.edu/president' || description_URL[i] == 'https://www.iit.edu/provost'){
            descriptions.push(document.querySelector('.checkerboard-item__description p').textContent.replace(/\n/g, ' ').trim());
        }
        else if(description_URL[i] == 'https://www.iit.edu/title-ix'){
            descriptions.push(document.querySelector('.hero__content__inner p').textContent.trim());
        }
        else if(description_URL[i] == 'https://web.iit.edu/student-employment/welcome'){
            document.querySelectorAll('.col-xs-12').forEach(e=>{
                if(e.textContent.includes('The Student Employment Office (SEO) oversees on-campus')){
                    descriptions.push(e.textContent);
                }
            })
        }
